...
const ImageMetadataViewer = () => {
  ...
  const { imageUrl, metadata, error } = state;
  ...
  const tailwind = {
    input: {
      ...
      imgPreview:
        "border-2 border-gray-500 rounded-[10px] h-[70%] flex content-center justify-center flex-col mx-0 my-2.5 p-[5px]",
  ...
  return (
    <ToolBoxLayout classNames="!min-w-auto max-xl:block">
      <ToolBox title="Input" mainClass="max-xl:!w-full max-xl:h-1/2">
        <input
          type="file"
          ref={imgInputRef}
          accept="image/*"
        ...
        <div className={tailwind.input.imgPreview}>
          {imageUrl === "" ? (
            ...
            <img src={imageUrl} className={tailwind.input.imgPreviewImg} />
          )}
        </div>
      </ToolBox>

      <ToolBox title="Output" mainClass="max-xl:!w-full max-xl:h-[45%]">
        <div className={tailwind.output}>
          {error ? (
            ...
            <JsonSyntaxHighlighter style={a11yDark} language="json">
