...
const constantTailwind = {
  heading:
    "text-center text-xl underline decoration-blue-800	decoration-2 underline-offset-[3.2px]",
  imgInfo: {
    main: "flex mb-6 text-start",
    title: "w-1/5",
    value:
      "border-b-[1px] w-[65%] text-center px-[30px] py-[1.5px] overflow-hidden text-ellipsis",
  },
  blurhashCanvas: {
    error: "text-red-600",
...
const ImgPlaceholderGen = () => {
  ...
  const [outputOptionsRadio, setOutputOptionsRadio] = useLocalStorageState(
    "ImgPlaceholderGen.:outputOptionsRadio",
    "color"
  );
  const [localBlurhash, setLocalBlurhash] = useLocalStorageState(
    "ImgPlaceholderGen.:localBlurhash",
    { blurhash: "", width: 0, height: 0 }
  );
  ...
  const isExtractedColorsBtnsDisabled =
    selectedColor == undefined || colorPalette.length === 0;

  const isTooltipVisibleForDimensionChange =
    isBlurhash.editing &&
    localBlurhash.blurhash != blurhash &&
    blurhashValidation.isValid;
  const isTooltipHiddenForMatchingDimensions =
    tmpCanvasDimensions.width === localBlurhash.width &&
    tmpCanvasDimensions.height === localBlurhash.height;
  const shouldShowDimensionTooltip =
    isTooltipVisibleForDimensionChange && isTooltipHiddenForMatchingDimensions;
  const canvasDimensions = {
    width: imgDimensions.width
      ? imgDimensions.width
      : isBlurhash.edited
      ? localBlurhash.width
      : tmpCanvasDimensions.width,
    height: imgDimensions.height
      ? imgDimensions.height
      : isBlurhash.edited
      ? localBlurhash.height
      : tmpCanvasDimensions.height,
  };
  ...
  const handleEditSaveBlurhash = () => {
    if (blurhashValidation.isValid === false) {
      ...
      if (blurhashProcessedStats.counts >= 1 || isBlurhash.edited) {
        ...
        updateObjectValue("tmpCanvasDimensions", {
          width: imgDimensions.width,
          height: imgDimensions.height,
        });
      }
      ...
      if (localBlurhash.blurhash !== blurhash) {
        ...
        setLocalBlurhash({
          blurhash,
          width: tmpCanvasDimensions.width,
          height: tmpCanvasDimensions.height,
        });
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      if (outputOptionsRadio === "blurhash") {
        ...
        updateValues({
          // clear the color cards and pallete
          dominatedColor: "not_set",
          averageColor: "not_set",
          imgSelectedColor: "not_set",
          colorPalette: {},
  ...
  useEffect(() => {
    ...
    updateObjectValue("blurhashValidation", {
      isValid: result,
  ...
  const tailwind = {
    ...
    input: {
      ...
      imgPreview:
        "border-2 border-gray-500 rounded-[10px] h-[62%] flex content-center justify-center flex-col mx-0 my-2.5 p-[5px]",
    ...
    output: {
      color: {
        colorPalette: {
          ...
          bottomBox:
            "flex justify-between px-4 pr-8 py-2 bg-[rgb(48,48,49)] rounded-b-lg",
        },
      },
      blurhash: {
        ...
        tmpDimensionsTooltipTriangle:
          "relative left-[-9px] top-6 inline-block w-0 h-0 border-y-[7px] border-y-transparent border-r-[14px] border-r-orange-500 rotate-[50deg]",

        tmpDimensionsWidthHeigt: cn(
          "flex w-[30%] rounded-lg border-b-0",
          shouldShowDimensionTooltip
            ? "border border-orange-400"
            : "border border-transparent"
        ),
        blurhashEditButtonsDiv: "flex justify-around	mb-3.5 mt-[5px]",
        blurhashCanvasDiv:
          "border-2 border-gray-500 rounded-[10px] h-[54%] flex content-center justify-center flex-col my-2.5 mx-0 p-[5px]",
        suggestions: cn(
        ...
        suggestion: cn("pb-3", imgSize > 768000 ? "block" : "hidden"),
            ...
            <div className={tailwind.input.outoputOptions.options}>
              {["color", "blurhash"].map((option, key) => (
                ...
                >
                  <label
                    className={
                      tailwind.input.outoputOptions.optionLableAndInput
                    }
                    htmlFor={option}
                  >
                    {option}
                  ...
                  <span
                    className={cn(
                      "grow",
                      isChangingTab && option !== outputOptionsRadio
                        ? "visible"
                        : "invisible"
                    )}
                  >
            ...
            <div className={tailwind.input.outoputOptions.optionBlurhashInputs}>
              {Object.entries(blurhashComponent).map(([input, value], key) => (
                <Input
                  ...
                  value={value}
                  onChange={(e) =>
                    updateObjectValue("blurhashComponent", {
            ...
            {[
              {
                ...
                about:
                  "Extracts the dominant color from an image by analyzing it's pixel data.",
                colorRGB: dominatedColor,
              },
              {
                title: "Average Color",
                about:
                  "Calculates the average color of an image by averaging the red, green, and blue values of all pixels in the image.",
              ...
              ({ title, about, colorRGB, conditionallyAppendText }, key) => (
                <ColorCard
                  key={key}
                  title={title}
                  about={about}
            ...
            <div className={tailwind.output.color.colorPalette.main}>
              <div className={tailwind.output.color.colorPalette.topBox.main}>
                <div className={tailwind.output.color.colorPalette.topBox.left}>
                  <h2
                    className={tailwind.output.color.colorPalette.topBox.title}
                  >
                    Extracted Color Palette
                  </h2>
                  <p
                    className={tailwind.output.color.colorPalette.topBox.about}
                  >
                    Find the most prominent colors. Output is a list of dominant
                    colors.
                  </p>
                  <div
                    className={
                      tailwind.output.color.colorPalette.topBox.paletteDiv
                    }
                  >
                    ...
                    {[...Array(6)].map((_, index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className={
                            tailwind.output.color.colorPalette.topBox.colorBox
                          }
                          style={{ backgroundColor: color }}
                          onClick={() =>
                            updateValues({ colorPaletteSelected: index })
                          }
                        >
                          <input
                            type="radio"
                            ...
                            className={
                              tailwind.output.color.colorPalette.topBox
                                .paletteRadio
                            }
                            readOnly
                          />
                ...
                <div
                  className={tailwind.output.color.colorPalette.topBox.right}
                >
                  <div
                    className={
                      tailwind.output.color.colorPalette.topBox.jsonPreview
                    }
                  >
                    {Object.keys(colorPalette).length !== 0 ? (
                      ...
                      <div
                        className={
                          tailwind.output.color.colorPalette.topBox.noImageDiv
                        }
                      >
                        <div
                          className={
                            tailwind.output.color.colorPalette.topBox.noImage
                          }
                        >
                          No image selected
                        </div>
              ...
              <div className={tailwind.output.color.colorPalette.bottomBox}>
                ...
                ].map(({ btnText, copyText, isLargeButton }, key) => (
                  <CopyBtn
                    key={key}
                    btnText={btnText}
                    copyText={copyText}
                    styles={{
                      width: isLargeButton ? "240px" : "180px",
                      height: "42px",
                    }}
          ...
          <>
            <div>
              <div className="flex">
                ...
                {isBlurhash.editing && (
                  <div
                    className={tailwind.output.blurhash.tmpDimensionsWidthHeigt}
                  >
                    {Object.entries(tmpCanvasDimensions).map(
                      ([input, value], key) => (
                        <Input
                          key={key}
                          value={value}
              ...
              <div className={tailwind.output.blurhash.blurhashEditButtonsDiv}>
                <CopyBtn copyText={blurhash} disabled={Boolean(!blurhash)} />
                <PasteBtn
                  onPaste={handlePaste}
                  btnDisabled={!isBlurhash.editing}
                />
                {[
                  ...
                  {
                    btnText: !isBlurhash.editing
                      ? "Edit"
                      : blurhashValidation.isValid === false
                      ? "Cancel"
                      : "Save",
                    onClick: handleEditSaveBlurhash,
                    classNames: "w-44",
                  },
                ].map(({ btnText, btnDisabled, onClick, classNames }, key) => (
                  <Btn
                    key={key}
                    btnText={btnText}
            ...
            <div className="flex">
              <div
                className={tailwind.output.blurhash.suggestions}
                onScroll={() =>
                  updateValues({ isSuggestionBoxScrolling: true })
                }
                onMouseLeave={() =>
                  updateValues({ isSuggestionBoxScrolling: false })
                }
              >
                <p className={tailwind.output.blurhash.suggestion}>
                  {/* <p className='pb-3'> */}
                  💡 The image size is {formatFileSize(imgSize)}. It's
                  recommended to compress the image before using it on the
                  website and generating a Blurhash.
                </p>
              ...
              <div
                className={tailwind.output.blurhash.testImgAndCopyJsonBtnsDiv}
              >
                <Btn
                  classNames="!w-[155px]"
                  btnText={"Test Img"}
...
const ImgInfo = ({ title, condition, value }) => {
  return (
      ...
      <div className={tailwind.infoAndBtns}>
        ...
        <p className={tailwind.about}>
          ...
          {conditionallyAppendText.length !== 0 && (
            <span className="text-white">{" " + appendText}</span>
          )}
        </p>
      ...
      return (
        <div className={constantTailwind.blurhashCanvas.error}>
          <div>
            Error:{" "}
            {error.message ||
              "Something went wrong while rendering the Blurhash."}
          </div>
